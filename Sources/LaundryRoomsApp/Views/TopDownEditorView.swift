...
struct TopDownEditorView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack {
                ...
                Canvas { context, _ in
                    drawPlan(context: &context, transform: transform)
                }
    ...
    private func drawPlan(context: inout GraphicsContext, transform: PlanTransform) {
        ...
        drawFixedCore(context: &context, transform: transform)

        for wall in store.plan.partitions {
            drawWall(wall, context: &context, transform: transform, preview: false)
        }
        for door in store.plan.doors {
            drawDoor(door, context: &context, transform: transform)
        }

        if let start = dragStart, let end = dragCurrent {
            drawWall(PartitionWall(start: start, end: end), context: &context, transform: transform, preview: true)
        }

        drawDimensions(context: &context, transform: transform)
    }

    private func drawGrid(context: inout GraphicsContext, transform: PlanTransform) {
        let step: Float = 0.5
        var path = Path()
        var x: Float = 0
        while x <= store.plan.dimensions.roomWidth + 0.001 {
            path.move(to: transform.point(PlanPoint(x: x, z: 0)))
            path.addLine(to: transform.point(PlanPoint(x: x, z: store.plan.dimensions.roomLength)))
            x += step
        }
        var z: Float = 0
        while z <= store.plan.dimensions.roomLength + 0.001 {
            path.move(to: transform.point(PlanPoint(x: 0, z: z)))
            path.addLine(to: transform.point(PlanPoint(x: store.plan.dimensions.roomWidth, z: z)))
            z += step
        }
        context.stroke(path, with: .color(.secondary.opacity(0.12)), lineWidth: 0.5)
    }

    private func drawWall(_ wall: PartitionWall, context: inout GraphicsContext, transform: PlanTransform, preview: Bool) {
        var path = Path()
        path.move(to: transform.point(wall.start))
        path.addLine(to: transform.point(wall.end))
        let selected = store.selectedWallID == wall.id
        let color: Color = preview ? .orange : (selected ? .accentColor : .primary)
        context.stroke(path, with: .color(color.opacity(preview ? 0.8 : 1)), style: StrokeStyle(lineWidth: max(4, transform.scale * CGFloat(store.plan.dimensions.drywallThickness)), lineCap: .square, dash: preview ? [8, 5] : []))
    }

    private func drawDoor(_ door: DoorOpening, context: inout GraphicsContext, transform: PlanTransform) {
        guard let wall = store.plan.partitions.first(where: { $0.id == door.wallID }), wall.length > 0 else { return }
    ...
    private func drawWindows(context: inout GraphicsContext, transform: PlanTransform) {
        let width = store.plan.dimensions.roomWidth
        ...
        let rearLeft = transform.point(PlanPoint(x: 0.30, z: store.plan.dimensions.roomLength))
        let rearRight = transform.point(PlanPoint(x: min(2.45, width - 0.3), z: store.plan.dimensions.roomLength))
        var rear = Path()
    ...
    private func drawFixedCore(context: inout GraphicsContext, transform: PlanTransform) {
        let d = store.plan.dimensions
    ...
    private func drawDimensions(context: inout GraphicsContext, transform: PlanTransform) {
        let d = store.plan.dimensions
        context.draw(
