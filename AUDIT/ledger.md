...
Totals: **66 tasks** — done:66 open:0 blocked:0
...
#### T0060 — Tracker T-01 (ship 10.8): deployed to the production VPS
...
- **fix**: Run at the owner's instruction, after the block below was cleared. The owner supplied the host's root password once; it was used for exactly one ssh to append this machine's public key (node1@Node1.local) to /root/.ssh/authorized_keys and was never written to the repository, the ledger or any log, and the temporary expect script that carried it was deleted. The deploy itself then ran over key auth: `cd roomcad/server && ./deploy.sh`, which syncs web/ and the new roomcad_api/ package with --delete, installs server.py, validates the candidate Caddyfile with the VPS's own Caddy before installing anything, sets file ownership for the roomcadapp service account, restarts the API, reloads and health-checks both Caddy layers, and refuses to report success until the live URL answers.
- **evidence (after)**: Deployed from main at 037c17c. Verified independently of the script's own report: https://roomcad.91.99.176.243.nip.io/version.js serves APP_VERSION "10.8"; /city/weather.js answers 200, so the city split is live and not just the entry point; / and /city.js answer 200; /api/rooms answers 401 unauthenticated (healthy). On the host: server.py is 81 lines, roomcad_api/ is 10 modules, and roomcad, roomcad-caddy and caddy are all active. The API was driven end to end — login, then a SECOND request on the SAME keep-alive connection — both directly against 127.0.0.1:8078 and through Caddy over TLS: the login declares its real Content-Length, the body parses, and the next response is intact. That is precisely the scenario T0065 was about, so the S1 fix is confirmed in production rather than only in the suite. Data is untouched: 35 rows and 3 distinct rooms (AB1, AB_Test, Maria1), identical to the pre-deploy backup of 2026-09-16, and rooms.db is chowned to roomcadapp with its WAL sidecars. Rollback taken before the deploy: /var/roomcad/backups/{server.py,web.tar.gz,Caddyfile,roomcad.caddy}.pre-10.8-20260919-045051.
- **commit**: 037c17c
