...
- **evidence (after)**: Raw-socket probe: Content-Length 12, exactly 12 bytes on the wire, connection stays open and the next request is intact. tests/server-live.test.py gained three checks (138 checks, was 134): the login body is the whole JSON object, the response body is exactly the length it declared (read on a raw socket, because http.client cannot see this), and a second request on the same keep-alive connection returns a parsable body. Both new framing checks were proved to fail on the old code (136 passed, 2 failed) and pass on the new. ruff check/format clean.
- **commit**: ca9c2e6
