...
class CityGenerator {
	...
	build( materials = {} ) {
		...
		const clearance = this.parameters.clearance;
		const slabs = [];
			...
			for ( let bz = 0; bz < L.blocksZ; bz ++ ) {
				...
				for ( let lx = 0; lx < L.lotsX; lx ++ ) {
					...
					for ( let lz = 0; lz < L.lotsZ; lz ++ ) {
						...
						// skip lots inside the central clearance so the room sits in the clear
						const lotCenterX = blockX + ( lx + 0.5 ) * L.lot;
						const lotCenterZ = blockZ + ( lz + 0.5 ) * L.lot;
						if ( clearance && Math.abs( lotCenterX ) < clearance.halfW && Math.abs( lotCenterZ ) < clearance.halfL ) continue;
...
CityGenerator.defaults = {
	...
	curbRadius: 5,
	clearance: null // { halfW, halfL } — a central rectangle (city-local) left empty for the room
};
