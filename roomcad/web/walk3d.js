...
import { CityGenerator } from "./lib/CityGenerator.js";
...
// The downtown city is generated by lib/CityGenerator.js.
...
export class Walk3D {
  constructor(container) {
    ...
    this.reusableTextures = new Set([this.skyTexture, this.cityGenerator.groundTexture]);
    for (const f of this.cityGenerator.facades) {
  ...
  /// A procedural downtown city generated by CityGenerator, confined to
  /// ~200 m around the canvas and purely visual (no physics colliders).
  buildCity(room) {
    const canvas = P.canvasOf(room);
    const group = this.cityGenerator.generate(
      canvas.width / 2, canvas.length / 2,
      canvas.width / 2 + 5, canvas.length / 2 + 5,
      200
    );
